...
func _ready() -> void:
	...
	LevelBuilder.build(self, level, _collected_ids, has_background, _items)
...
func _compute_tracks(player_pos: Vector2) -> Dictionary[String, ProximityTrack]:
	...
	for status in statuses:
		...
		if quest.kind == "reach":
			...
			tracks[quest.id] = ProximityTrack.new(GoalProximity.tier_for(dist, prox.at, prox.near, prox.mid), dist)
			continue
		...
		if quest.kind == "collect":
			...
			tracks[quest.id] = ProximityTrack.new(GoalProximity.tier_for(nearest_dist, prox2.at, prox2.near, prox2.mid), nearest_dist)
	return tracks
...
func _add_decoration_sprite(obj_id: String, texture_path: String, tile_rect: Rect2i, z: int) -> void:
	...
	sprite.scale = Vector2(target_obj.rect.size.x / tile_rect.size.x, target_obj.rect.size.y / tile_rect.size.y)
