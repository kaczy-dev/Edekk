...
## zapłacie") — `gate_id` identity is the placed ShortcutGate's own export,
## same convention as `obj_id` elsewhere in this file (author-assigned,
## stable string, not an autogenerated node path). A gate stays unlocked
## forever once opened — no re-lock path exists, same "permanent progress"
## shape as `unlocked_levels`.
var unlocked_shortcuts: Array[String] = []


func is_shortcut_unlocked(gate_id: String) -> bool:
	return unlocked_shortcuts.has(gate_id)


func unlock_shortcut(gate_id: String) -> void:
	if unlocked_shortcuts.has(gate_id):
		return
	unlocked_shortcuts.append(gate_id)
	save_progress()
...
func load_progress() -> void:
	...
	var loaded_shortcuts: Array[String] = []
