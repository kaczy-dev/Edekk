...
func test_player_attack_out_of_range_does_nothing() -> void:
	...
	var enemy_health: HealthComponent = _enemy.get_node("HealthComponent")
	assert_eq(enemy_health.current_hp, 12, "an attack that never overlaps the enemy deals no damage")
...
func test_enemy_dies_and_is_freed_after_death_animation() -> void:
	...
	assert_false(is_instance_valid(_enemy), "EnemyActor frees itself once the death animation finishes")
